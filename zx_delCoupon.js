...
!(async () => {
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    zxObject.cookie = cookiesArr[i];
    if (zxObject.cookie) {
      if (i) console.log(`\n***************开始京东账号${i + 1}***************`)
      await initial()
      //await zxObject.queryJdUserInfo();
...
function dealCouponList(data,timeout = 0){
  return new  Promise((resolve) => {
    setTimeout(async () => {
      try {
        if (zxObject.printDetail) console.log(JSON.stringify(data))
        for (let i in data.coupon.useable) {
          ...
          if (data.coupon.useable[i].coupontype === 1) {
    ...
    setTimeout( ()=>{
      ...
      $.post(url, async (err, resp, data) => {
        try {
          ...
          if (zxObject.printDetail) console.log(data)
          data = JSON.parse(data)
          if (data.retcode) {
