...
//先筛选，等待时间后兑换， 先兑换一次1000的，再分别兑换10次1都的

const zxCommon = require('./zx_common.js');
let zxObject = new zxCommon.ZxObject('东东超市兑换奖品');
const $ = zxObject.$;
...
!(async () => {
    ...
    //获取数据          
    await smtg_queryPrize($.cookiesArr[0]);
    await $.dowork(async function () {
        await smtgHome($.index); //查询蓝币数量，是否满足兑换的条件
    })
...
async function PrizeIndex(configCost) {
    ...
    if (configCost == 1000) {
        ...
        if (totalBlueArr && totalBlueArr[$.index] && totalBlueArr[$.index] > $.blueCost) {
            await smtg_obtainPrize(prizeList[1].prizeId);
        } else {
            console.log(`兑换失败,您目前蓝币${totalBlueArr[$.index]}个,不足以兑换${$.title}所需的${$.blueCost}个`);
            $.beanerr = `兑换失败,您目前蓝币${totalBlueArr[$.index]}个,不足以兑换${$.title}所需的${$.blueCost}个`;
        }
    } else if (configCost == 1) {
    ...
    //         }
    //         //兑换1000京豆
    //         if (totalBlueArr[$.index] > $.blueCost) {
    //             for (let j = 0; j <= 3; j++) {
    //                 smtg_obtainPrize(prizeList[1].prizeId, cookie, userName);
    //                 if ($.errBizCodeCount >= 3) break
    //             }
    //         } else {
    //             console.log(`兑换失败,您目前蓝币${totalBlueArr[$.index]}个,不足以兑换${$.title}所需的${$.blueCost}个`);
    ...
    //         if (totalBlueArr[$.index] > $.blueCost) {
    //             for (let j = 0; j <= 10; j++) {
    //                 smtg_obtainPrize(prizeList[0].prizeId, $.cookie, userName, 1000);
    //                 if ($.errBizCodeCount >= 3) break
    //             }
    //         } else {
    //             console.log(`兑换失败,您目前蓝币${totalBlueArr[$.index]}个,不足以兑换${$.title}所需的${$.blueCost}个`);
    ...
    //                 if ($.type === 4 && !$.beanType) {
    //                     for (let j = 0; j <= 10; j++) {
    //                         await smtg_obtainPrize(prizeId, cookie, userNamem, 0, "smtg_lockMaterialPrize")
    //                         if ($.errBizCodeCount >= 3) break
    //                     }
    //                 } else {
    //                     for (let j = 0; j <= 10; j++) {
    //                         await smtg_obtainPrize(prizeId, cookie, userName);
    //                         if ($.errBizCodeCount >= 3) break
    //                     }
    //                 }
    //             } else {
...
//查询任务
function smtg_queryPrize(cookie, timeout = 0) {
    $.queryPrizeData = [];
    return new Promise((resolve) => {
        setTimeout(() => {
            let url = {
            ...
            $.post(url, async (err, resp, data) => {
                try {
                    ...
                    if ($.safeGet(data)) {
                        ...
                        if ($.data.data.bizCode === 0) {
                            $.beanscount++;
                            console.log(`【京东账号${$.nickName || $.UserName} 第${$.data.data.result.count}次换${$.title}成功`)
                ...
                if (err) {
                    ...
                    if ($.safeGet(data)) {
                        ...
                        if (data.data.bizCode === 0) {
                            ...
                            let totalBlue = result.totalBlue;
                            totalBlueArr[$.index] = totalBlue;
                            console.log(`${i} 【总蓝币】${totalBlue}个\n`);
                        }
                    }
                }
            } catch (e) {
