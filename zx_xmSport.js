...
async function start(user, password, login_token, step) {
    //let login_token = $.isNode() ? (process.env.XM_SPORT_TOKEN ? process.env.XM_SPORT_TOKEN : login_token) : ($.getdata('xmSportsToken') ? $.getdata('xmSportsToken') : login_token);
    ...
    if (login_token) {
        ...
        if ($.tokenInfo && $.tokenInfo.result === 'ok') {
            ...
            await get_time();
            await change_step(app_token, user_id, step);
            if ($.changeStepRes && $.changeStepRes.code === 1) {
                console.log(`步数修改成功:${step}步`);
                allMessage += `步数修改成功:${step}步\n`
...
function change_step(app_token, user_id, step) {
